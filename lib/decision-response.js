 ...
 */
var DecisionResponse = function (swfClient, taskToken, defaultTaskList, logger) {
  ...
  send: function(cb) {
   ...
   fail_workflow_execution: function (reason, details, cb) {
      ...
      this.respondCompleted([{
         ...
         if (err) { self.logger.error(err); return; }
         self.logger.error("Workflow marked as failed ! (decision task)");
      });
   },
